...
const register = async () => {
  const respond = await supabase.auth.signUp({
    email: insert_email.value,
...
<template>
  <div class="flex h-screen w-full relative">
    <div class="hidden lg:flex lg:w-1/2 relative bg-slate-200 dark:bg-slate-800 overflow-hidden">
      <div class="absolute inset-0 w-full h-full bg-cover bg-center transition-transform duration-700 hover:scale-105"
      ...
      <div
        ...
        <div class="max-w-lg text-left">
          ...
          <h2 class="text-[3.25rem] font-bold leading-tight mb-4 drop-shadow-sm">Everything your pet needs, right at
            your fingertips.</h2>
          <p class="text-lg text-slate-200 font-medium leading-relaxed drop-shadow-sm">Track vaccinations, schedule vet
            appointments, and manage your furry friend's diet all in one place.</p>
        </div>
      </div>
    </div>
    <div
      class="w-full lg:w-1/2 flex flex-col h-full bg-white dark:bg-background-dark relative shadow-2xl lg:shadow-none overflow-y-auto">
      <header
        ...
        <router-link to="/"
          ...
          <svg viewBox="0 0 24 24" width="24px "
            class="group-hover:-translate-x-4 duration-300 transition group-hover:fill-white" fill="none"
            xmlns="http://www.w3.org/2000/svg">
            <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
            <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"></g>
            <g id="SVGRepo_iconCarrier">
              <path d="M4 12H20M4 12L8 8M4 12L8 16" stroke="#485364" stroke-width="2" stroke-linecap="round"
        ...
        <div class="w-full max-w-[420px] space-y-8">
          <div class="bg-slate-100 dark:bg-slate-800/50 p-1.5 rounded-xl flex relative">
            ...
            <button type="button" @click="authMode = 'login'"
              class="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-200" :class="authMode === 'login'
            ...
            <button type="button" @click="authMode = 'register'"
              class="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all duration-200" :class="authMode === 'register'
                ? 'bg-[#19a1e6] text-white shadow-md shadow-[#19a1e6]'
                : 'text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-slate-200'">
              Register
            </button>
          ...
          <div class="relative overflow-y-hidden transition-[min-height] h-max min-h-[420px] duration-300 ease-in-out">
            <transition name="auth-fade-slide" mode="out-in">

              <!-- LOGIN -->
              <form v-if="authMode === 'login'" key="login" @submit.prevent="login"
                ...
                <div class="space-y-1.5">
                  <div class="flex justify-between items-center">
                    ...
                    <a class="text-xs font-bold text-primary hover:text-primary/80 transition-colors" href="#">Forgot
                      Password?</a>
                  </div>
                  <div class="relative group">
                    <div class="absolute inset-y-0 left-0 pl-3.5 flex items-center pointer-events-none">
              ...
              <form v-else key="register" @submit.prevent="register" class="absolute inset-0 space-y-5 text-left px-4">
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    ...
                    <input v-model="insert_email"
                      class="w-full h-12 pl-11 pr-4 bg-slate-50 dark:bg-slate-800 border border-[#d0dfe7] dark:border-slate-700 rounded-lg text-base text-[#0e171b] dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                      id="insert_email" placeholder="name@example.com" required type="email">
                  </div>
                </div>
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    ...
                    <input v-model="insert_password"
                      class="w-full h-12 pl-11 pr-12 bg-slate-50 dark:bg-slate-800 border border-[#d0dfe7] dark:border-slate-700 rounded-lg text-base text-[#0e171b] dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                      id="insert_password" placeholder="••••••••" required type="password">
                    <button
                      class="absolute inset-y-0 right-0 pr-3.5 flex items-center cursor-pointer text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
                      type="button">
                ...
                <div class="space-y-1.5">
                  ...
                  <div class="relative group">
                    ...
                    <input v-model="insert_password_confirmation"
                      class="w-full h-12 pl-11 pr-12 bg-slate-50 dark:bg-slate-800 border border-[#d0dfe7] dark:border-slate-700 rounded-lg text-base text-[#0e171b] dark:text-white placeholder:text-slate-400 focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all"
                      id="insert_password_confirmation" placeholder="••••••••" required type="password">
                  </div>
                </div>
                <button
          ...
          <div class="relative py-2">
            ...
            <div class="relative flex justify-center">
              <span
                class="bg-white dark:bg-background-dark px-4 text-xs font-medium text-slate-500 uppercase tracking-wider">Or
                continue with</span>
            </div>
          </div>
          <div class="grid grid-cols-2 gap-4">
            <button
              class="flex items-center justify-center gap-2.5 px-4 py-2.5 border border-[#d0dfe7] dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all text-[#0e171b] dark:text-slate-200 font-bold text-sm group">
              <img alt="Google Logo" class="w-5 h-5 transition-transform group-hover:scale-110"
                data-alt="Google G Logo Color"
                src="https://lh3.googleusercontent.com/aida-public/AB6AXuAAddrZilh0cyBxlpT0utn38MLnp_HcZzYrntxq6RONW2j844Yd8KsbIwctlQUw37lPTFbbGqTMBMiiXP_LqteXVWVOZEoLC2SN4hSWbuWvZR43BeOJIloxzANa_D_f7omXK276Em1JFdh7g6LxiABBq5re2gi-QcUnldcw7KZKM2UXEido7Tnz7XWGONboHJ7eamkSRrvOCB739mMa4n6H2KDmWqVgsPu9NDi6m4M-c66vljWs_JYMlf9kbOFaGcTSLZo8N3SiubOh">
              <span>Google</span>
            </button>
            <button
              class="flex items-center justify-center gap-2.5 px-4 py-2.5 border border-[#d0dfe7] dark:border-slate-700 rounded-lg bg-white dark:bg-slate-800 hover:bg-slate-50 dark:hover:bg-slate-700 hover:border-slate-300 dark:hover:border-slate-600 transition-all text-[#0e171b] dark:text-slate-200 font-bold text-sm group">
  ...
  <transition name="scale">
    <div v-if="showErrorModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="bg-white dark:bg-slate-900 rounded-xl p-6 w-[90%] max-w-sm text-center">
        ...
        <button @click="showErrorModal = false" class="mt-5 px-6 py-2 bg-red-500 text-white rounded-lg">
          Cerrar
        </button>
      </div>
    </div>
  ...
  <transition name="scale">
    <div v-if="showSuccessModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
      <div class="flex flex-col bg-white dark:bg-slate-900 rounded-xl p-6 w-[90%] max-w-sm text-center ">
        <h3 class="text-lg font-bold text-green-600 mb-2">Login exitoso 🎉</h3>
        <p class="text-slate-600 dark:text-slate-300">
          {{ welcomeMessage }}
        </p>
        <router-link to="/" class=" px-6 py-2 bg-[#90ee90] hover:bg-[#43d743] text-white rounded-lg mt-4">
          ir a home
        </router-link>
      </div>
    </div>
